...
BOARD_SIZE = 4
MAX_ONEHOT_EXPONENT = 15  # channels: 0(empty), 1(2), ..., 15(32768+ clipped)
OBS_CHANNELS = MAX_ONEHOT_EXPONENT + 1
...
def extract_obs(state, player_id=0):
    ...
    # One-hot planes keep tile identity explicit for local merge detection by Conv2D.
    ...
    exponents = np.clip(exponents, 0, MAX_ONEHOT_EXPONENT)

    one_hot = np.eye(OBS_CHANNELS, dtype=np.float32)[exponents]  # (4, 4, C)
    return np.moveaxis(one_hot, -1, 0)  # (C, 4, 4)
...
class OpenSpiel2048Env:
    def __init__(
        ...
        self.obs_shape = (OBS_CHANNELS, BOARD_SIZE, BOARD_SIZE)
    ...
    def step(self, action):
        ...
        next_obs = extract_obs(self.state, self.player_id) if not self.state.is_terminal() else np.zeros(self.obs_shape, dtype=np.float32)
...
def build_n_step_transition(n_step_queue, gamma):
    """Compress a queue of 1..N raw transitions into a single n-step transition."""
    first = n_step_queue[0]

    discounted_reward = 0.0
    final_next_obs = first.next_obs
    final_done = first.done
    final_next_legal_mask = first.next_legal_mask

    for i, tr in enumerate(n_step_queue):
        discounted_reward += (gamma ** i) * float(tr.reward)
        final_next_obs = tr.next_obs
        final_done = tr.done
        final_next_legal_mask = tr.next_legal_mask
        if tr.done:
            break
    ...
    return Transition(
        ...
        bool(final_done),
        first.legal_mask,
        final_next_legal_mask,
    )


def flush_n_step_queue_to_replay(n_step_queue, replay, gamma):
    """Flush remaining 1..N-1 transitions at episode end."""
    while n_step_queue:
        n_step_transition = build_n_step_transition(n_step_queue, gamma)
...
REWARD_SHAPING_CONFIG = {
    "target_corner": "top_left",
    # Scale raw score deltas to the same range as shaping terms for stable n-step targets.
    "score_weight": 0.005,
    "empty_weight": 0.05,
    "monotonicity_weight": 0.5,
    "corner_weight": 1.0,
...
def dqn_update(batch):
    ...
    with torch.no_grad():
        ...
        target = rewards + N_STEP_DISCOUNT * next_q_target
...
for episode in tqdm(range(1, NUM_EPISODES + 1), desc="Training (shaped reward)"):
    ...
    while not done and ep_len < MAX_STEPS_PER_EPISODE:
        ...
        n_step_queue.append(
            Transition(obs, action, reward, next_obs, done, legal_mask, next_legal_mask)
        )
    ...
    flush_n_step_queue_to_replay(n_step_queue, replay, GAMMA)
...
torch.save(
    {
        ...
        "obs_dim": obs_dim,
        "obs_shape": obs_shape,
        "max_onehot_exponent": MAX_ONEHOT_EXPONENT,
        "num_actions": num_actions,
        "n_step": N_STEP,
