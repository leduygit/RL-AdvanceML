...
class QNetwork(nn.Module):
    def __init__(self, obs_dim, num_actions, hidden_dim=256):
        super().__init__()
        self.board_size = int(np.sqrt(obs_dim))
        if self.board_size * self.board_size != obs_dim:
            raise ValueError(f"obs_dim={obs_dim} is not a perfect square for CNN board input.")
...
checkpoint_path = "double_dqn_cnn.pt"
torch.save(
    {
